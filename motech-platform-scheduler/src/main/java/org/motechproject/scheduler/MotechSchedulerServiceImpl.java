...
public class MotechSchedulerServiceImpl extends MotechObject implements MotechSchedulerService {
	...
	public void scheduleJob(CronSchedulableJob cronSchedulableJob) {
		...
		try {
			...
			Date triggerStartTime = startTime != null ? startTime : new Date();

			trigger = newTrigger()
					.withIdentity(triggerKey(jobId.value(), JOB_GROUP_NAME))
					.withSchedule(CronScheduleBuilder.cronSchedule(cronSchedulableJob.getCronExpression()))
	...
	public void rescheduleJob(String subject, String externalId, String cronExpression) {
		...
		try {
			schedulerFactoryBean.getScheduler().rescheduleJob(triggerKey(jobId.value(), JOB_GROUP_NAME), trigger);
		} catch (SchedulerException e) {
	...
	private void unscheduleJob(String jobId) {
		...
		try {
			schedulerFactoryBean.getScheduler().unscheduleJob(triggerKey(jobId, JOB_GROUP_NAME));
		} catch (SchedulerException e) {
	...
	public void unscheduleAllJobs(String jobIdPrefix) {
		...
		try {
			Set<TriggerKey> triggerKeys = schedulerFactoryBean.getScheduler().getTriggerKeys(GroupMatcher.triggerGroupEquals(JOB_GROUP_NAME));
