...
/**
 * This is a test class which test the validator methods in <code>HubValidator</code> class for publish/subscribe APIs
 * @author Anuranjan
 *
 */
...
public class HubValidatorTest {
	...
	@InjectMocks private HubValidator hubValidator = new HubValidator();
	...
	/**
	 * Invalid scenario: mandatory fields empty and <code>leaseSeconds</code> non-numeric
	 */
	@Test
	public void validateSubscriptionWithEmptyStringsAndInvalidLeaseSeconds() {
		...
		assertEquals("hub.callback must be provided", errors.get(0));
		assertEquals("hub.mode must be provided", errors.get(1));
		assertEquals("hub.topic must be provided", errors.get(2));
		assertEquals("hub.lease_seconds must be numeric. Provided value is [a]", errors.get(3));
	}
	...
	public void validateSubscriptionWithInvalidModes() {
		...
		assertEquals(3, errors.size());
		assertEquals("hub.callback must be provided", errors.get(0));
		assertEquals("Invalid mode type [publish]. Supported modes: 'subscribe' or 'unsubscribe'", errors.get(1));
		assertEquals("hub.topic must be provided", errors.get(2));
	}
	...
	public void validateSubscriptionWithException() {
		...
		assertEquals(3, errors.size());
		assertEquals("hub.callback must be provided", errors.get(0));
		assertEquals("hub.mode provided is not valid. Provided value is [inValid]", errors.get(1));
		assertEquals("hub.topic must be provided", errors.get(2));
	}
	
	/**
	 * Invalid scenario: <code>callback</code> and <code>topic</code> null and <code>secret</code> too long
	 */
	@Test
	public void validateSubscriptionWithValidModesAndOtherMandatoryFieldsNull() {
		...
		assertEquals(3, errors.size());
		assertEquals("hub.callback must be provided", errors.get(0));
		assertEquals("hub.secret is too long. Maximum 200 bytes supported. Provided length is [206]", errors.get(1));
		assertEquals("hub.topic must be provided", errors.get(2));
		
		mode = "unsubscribe";
		List<String> errors1 = hubValidator.validateSubscription(callbackUrl, mode, topic, leaseSeconds, secret);
	...
	/**
	 * Invalid scenario: <code>callback</code>, <code>mode</code> and <code>topic</code> must not be null
	 */
	@Test
	...
	/**
	 * Invalid scenario: <code>mode</code> and <code>url</code> must not be empty
	 */
	@Test
	...
	public void validatePingWithInvalidMode() {
		...
		assertNotNull(errors);
		assertEquals(2, errors.size());
		assertEquals("hub.mode provided is not valid. Provided value is [inValid]", errors.get(0));
		assertEquals("hub.url must be provided", errors.get(1));
	}

	/**
	 * Invalid scenario: Unsupported <code>mode</code> type and missing <code>url</code>
	 */
	@Test
	public void validatePingWithValidMode() {
		...
		assertNotNull(errors);
		assertEquals(2, errors.size());
		assertEquals("Invalid mode type [subscribe]. Supported mode: 'publish'", errors.get(0));
		assertEquals("hub.url must be provided", errors.get(1));
	}
	...
	public void validatePingWithValidParams() {
		...
		assertNotNull(errors);
		assertEquals(1, errors.size());
		assertEquals("Invalid mode type [unsubscribe]. Supported mode: 'publish'", errors.get(0));
	}
}
