...
public class ContentDistributionServiceImpl implements	ContentDistributionService {
	...
	public void distribute(String url) {
		...
		if(hubTopics == null || hubTopics.isEmpty() ) {
			...
			topicId = (int)hubTopicMDSService.getDetachedField(hubTopics.get(0), "id");
		}
		...
		if (response != null && response.getStatusCode().value() / 100 == 2) {
			...
			List<HubSubscription> subscriptionList = 
					hubSubscriptionMDSService.findByTopic(topicId);
			for (HubSubscription subscription : subscriptionList) {
				int subscriptionId = (int)hubSubscriptionMDSService.getDetachedField(subscription, "id");
				
				int retryCount = 0;
				DistributionStatusLookup statusLookup = DistributionStatusLookup.FAILURE;
				int subscriptionStatusId = subscription.getHubSubscriptionStatusId();
				if (subscriptionStatusId == 
						SubscriptionStatusLookup.INTENT_VERIFIED.getId()) {
				...
				subscriberTransaction.setHubDistributionStatusId(statusLookup.getId());
