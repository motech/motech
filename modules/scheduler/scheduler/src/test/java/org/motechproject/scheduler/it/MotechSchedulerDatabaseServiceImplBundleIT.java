...
public class MotechSchedulerDatabaseServiceImplBundleIT extends BasePaxIT {
    ...
    public void shouldFilterJobsByDate() {
        try {
            ...
            assertEquals("test_event_3-job_id3-cron", jobs.get(0).getName());
    ...
    public void shouldFilterJobsByName() {
        try {
            ...
            assertEquals(jobs.get(0).getName(), "test_event_1-job_id1-cron");
            ...
            assertNotNull(jobs);
            assertEquals(1, jobs.size());
            assertEquals(jobs.get(0).getName(), "test_event_2-job_id2-cron");
            rowCount = databaseService.countJobs(jobsSearchSettings);
            assertEquals(1, rowCount);
        } finally {
            stopFakingTime();
    ...
    public void shouldFilterJobsWithSortingAndPagination() {
        try {
            ...
            assertEquals(printJobNames(jobs), "test_event_1-job_id1-cron", jobs.get(0).getName());
            ...
            jobs = databaseService.getScheduledJobsBasicInfo(jobsSearchSettings);
            assertEquals(2, jobs.size());
            assertEquals(printJobNames(jobs), "test_event_3-job_id3-cron", jobs.get(0).getName());
