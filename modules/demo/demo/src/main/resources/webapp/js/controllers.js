(function () {
    ...
    /* Controllers */
    var demoModule = angular.module('motech-demo');

    demoModule.controller('TreeListCtrl', function ($scope, Tree, $http) {
    ...
    demoModule.controller('TreeCreateCtrl', function ($scope, $http) {
        $scope.tree = {
            name: '',
            description: '',
            nodes: [{
        ...
        $scope.validation = function () {
            ...
            if ($scope.tree.name === null || $scope.tree.name === '') {
                errMsg = jQuery.i18n.prop('trees.error.treeNameRequired');
            ...
            for (i = 0; i < $scope.tree.nodes.length; i += 1) {
                ...
                if (!node.name) {
                    errMsg = jQuery.i18n.prop('trees.error.nodeNameRequired');
                    errId = node.id + '.name';
                ...
                if (node.message === null || node.message === '') {
                    errMsg = jQuery.i18n.prop('trees.error.nodeMessageRequired');
                    errId = node.id + '.message';
                ...
                for (j = 0; j < node.transitions.length; j += 1) {
                    ...
                    if (transition.key === null || transition.key === '') {
                        errMsg =  jQuery.i18n.prop('trees.error.transitionKeyRequired');
                        errId = node.id + ".transition." + transition.id;
        ...
        $scope.getValidationError = function(nodeId, fieldName, transitionId) {
            ...
            errMsg = $scope.validationMsgs[errId];
    ...
    demoModule.controller('TreeExecuteCtrl', function ($scope, Tree, $routeParams) {
        $scope.entered = [];
        $scope.history = [];
    ...
    demoModule.controller('IVRCallCtrl', function ($scope, i18nService, $http) {
            ...
            if ($scope.delay === '0' || $scope.delay === null) {
                ...
                $http.get('../demo/api/scheduleCall?phone=' + $scope.phone + '&callDelay=' + $scope.delay).success(function() {
                    ...
                    var oldDate = new Date(),
                        newDate = new Date(oldDate.getTime() + $scope.delay*60000);
    ...
    demoModule.controller('EventCtrl', function ($scope, $http) {
