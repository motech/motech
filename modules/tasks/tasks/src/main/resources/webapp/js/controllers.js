(function () {
    ...
    controllers.controller('TasksDashboardCtrl', function ($scope, $filter, Tasks, Activities, $rootScope, $http, ManageTaskUtils, ModalServ) {
            ...
            dialog = new BootstrapDialog ({
                ...
                buttons: [{
                    ...
                    action: function(dialog) {
                        ...
                        ModalServ.alertHandler('task.error.removed', 'task.header.error')
                        );
                    }
                }]
            });
        ...
        $scope.importTask = function () {
            ...
            $('#importTaskForm').ajaxSubmit({
                ...
                error: function (response) {
                    ModalServ.handleResponse('task.header.error', 'task.error.import', response);
                }
            });
        };
    ...
    controllers.controller('TasksManageCtrl', function ($scope, ManageTaskUtils, Channels, DataSources, Tasks, Triggers, ModalServ, $q, $timeout, $routeParams, $http, $compile, $filter) {
        $scope.util = ManageTaskUtils;
        ...
        $scope.selectTrigger = function (channel, trigger) {
            if ($scope.task.trigger) {
                ModalServ.motechConfirm('task.confirm.trigger', "task.header.confirm", function (val) {
                    ModalServ.remove();
                    if (val) {
        ...
        $scope.removeAction = function (idx) {
            ...
            if ($scope.selectedActionChannel[idx] !== undefined && $scope.selectedActionChannel[idx].displayName !== undefined) {
                ModalServ.motechConfirm('task.confirm.action', "task.header.confirm", function (val) {
                    ModalServ.remove();
                    if (val) {
        ...
        $scope.removeFilterSet = function (data) {
            ...
            if (data.filters !== undefined && data.filters.length > 0) {
                ModalServ.motechConfirm('task.confirm.filterSet', "task.header.confirm", function (val) {
                    ModalServ.remove();
                    if (val) {
        ...
        $scope.removeData = function (dataSource) {
            if (dataSource.type !== undefined || (dataSource.providerName !== undefined && dataSource.providerName !== '')) {
                ModalServ.motechConfirm('task.confirm.dataSource', "task.header.confirm", function (val) {
                    ModalServ.remove();
                    if (val) {
        ...
        $scope.selectDataSource = function (dataSource, selected) {
            if (dataSource.providerId) {
                ModalServ.motechConfirm('task.confirm.changeDataSource', 'task.header.confirm', function (val) {
                    ModalServ.remove();
                    if (val) {
                        dataSource.name = '';
        ...
        $scope.selectObject = function (object, selected) {
            if (object.type) {
                ModalServ.motechConfirm('task.confirm.changeObject', 'task.header.confirm', function (val) {
                    ModalServ.remove();
                    if (val) {
                        object.name = '';
        ...
        $scope.save = function (enabled) {
            var success = function (response) {
                    ...
                    loc, indexOf, errors = response.validationErrors || response, dialog;
                ...
                error = function (response) {
                    var data = (response && response.data) || response, dialog;
    ...
    controllers.controller('TasksLogCtrl', function ($scope, Tasks, Activities, $routeParams, $filter, $http, ModalServ) {
        var data, task;
        ...
        $scope.clearHistory = function () {
            ModalServ.motechConfirm('task.history.confirm.clearHistory', 'task.history.confirm.clear',function (r) {
                ModalServ.remove();
                if (!r) {
                    return;
                }
                ...
                Activities.remove({taskId: $routeParams.taskId}, function () {
                     ...
                     ModalServ.unblockUI();
                     ModalServ.handleResponse('task.header.error', 'task.history.deleteError', response);
                 });
            });
        };
        ...
        $scope.retryTask = function (activityId) {
            $http.post('../tasks/api/activity/retry/' + activityId)
                .success(function () {
                    ModalServ.motechAlert('task.retry.info', 'task.retry.header');
                        ModalServ.remove();
                })
                .error(function() {
                    ModalServ.motechAlert('task.retry.failed', 'task.retry.header');
                        ModalServ.remove();
                });
        };
    });
    ...
    controllers.controller('TasksSettingsCtrl', function ($scope, Settings, ModalServ) {
        ...
        $scope.submit = function() {
            $scope.settings.$save(function() {
                ModalServ.motechAlert('task.settings.success.saved', 'server.saved');
                    ModalServ.remove();
            }, function() {
                ModalServ.motechAlert('task.settings.error.saved', 'server.error');
                    ModalServ.remove();
            });
        };
    ...
    controllers.controller('MapsCtrl', function ($scope, ModalServ) {
       ...
       */
       $scope.emptyMap = function (pair) {
        ...
        $scope.reset = function () {
            ...
            ModalServ.motechConfirm('task.confirm.reset.map', "task.header.confirm", function (val) {
                ModalServ.remove();
                if (val) {
                    resetMap();
                }
