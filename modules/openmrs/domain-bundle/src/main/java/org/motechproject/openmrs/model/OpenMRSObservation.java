 ...
 */
public class OpenMRSObservation<T> implements MRSObservation<T> {
    ...
    public void addDependantObservation(MRSObservation mrsObservation) {
        ...
        List<? extends MRSObservation> existingObservationList = Lambda.filter(having(on(OpenMRSObservation.class).getConceptName(), is(equalTo(mrsObservation.getConceptName()))), dependantObservations);
