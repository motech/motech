...
public class FlowSessionServiceImpl implements FlowSessionService {
    ...
    @Override
    public FlowSession createSession(String sessionId, String phoneNumber) {
        if (StringUtils.isBlank(sessionId)) {
            sessionId = UUID.randomUUID().toString();
        }
        FlowSession session = getSession(sessionId);
        if (session == null) {
            session = allFlowSessionRecords.findOrCreate(sessionId, phoneNumber);
        }
        allFlowSessionRecords.update((FlowSessionRecord) session);
        return session;
    }
    ...
    public FlowSession getSession(HttpServletRequest request, String language, String phoneNumber) {
        ...
        return copyParameters(request, createSession(sessionId, phoneNumber));
    }
