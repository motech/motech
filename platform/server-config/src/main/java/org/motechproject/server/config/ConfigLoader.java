...
public class ConfigLoader {
    ...
    public ConfigFileSettings loadConfig() {
        ...
        for (ConfigLocation configLocation : configLocations) {
            ...
            try {
                Resource motechSettings = configLocationResource.createRelative(PlatformSettingsService.SETTINGS_FILE_NAME);
                if (!motechSettings.isReadable()) {
                    LOGGER.warn("Could not read motech-settings.conf from: " + configLocationResource.toString());
                    continue;
                }

                Resource activemq = configLocationResource.createRelative(PlatformSettingsService.ACTIVEMQ_FILE_NAME);
