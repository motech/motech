(function () {
    ...
    controllers.controller('WebSecurityUserCtrl', function ($scope, Roles, Users, $http, ModalService) {
           $scope.user = {
               externalId : "",
               userName: "",
               password: "",
           ...
           $scope.saveUser = function() {
               $http.post('../websecurity/api/users/create', $scope.user).
                    success(function(){
                        ModalService.motechAlert('security.create.user.saved', 'security.create');
                        $scope.userList = Users.query();
                        $scope.showUsersView=!$scope.addUserView;
                        $scope.addUserView=!$scope.addUserView;
                    }).
                    error(function(response) {
                        ModalService.handleResponse('server.error', 'security.create.user.error', response);
           ...
           $scope.updateUser = function(){
               $http.post('../websecurity/api/users/update', $scope.user).
                   success(function(){ModalService.motechAlert('security.update.user.saved', 'security.update');
                       $scope.userList = Users.query();
                       $scope.showUsersView=!$scope.editUserView;
                       $scope.editUserView=!$scope.editUserView;
                       $scope.$emit('module.list.refresh');
                   }).error(function(response) {
                        ModalService.handleResponse('server.error', 'security.update.user.error', response);
                   });
           };
           ...
           $scope.deleteUser = function() {
               $http.post('../websecurity/api/users/delete', $scope.user).
                    success(function(){
                        ModalService.motechAlert('security.delete.user.saved', 'security.deleted');
                        $scope.showUsersView=!$scope.editUserView;
                        $scope.editUserView=!$scope.editUserView;
                        $scope.userList = Users.query();
                    }).error(function(){ModalService.motechAlert('security.delete.user.error', 'server.error');});
           };
    ...
    controllers.controller('WebSecurityRolePermissionCtrl', function ($scope, Roles, Permissions, $http, ModalService) {
           $scope.role = {
                roleName : '',
                originalRoleName:'',
                permissionNames : [],
           ...
           $scope.addPermission = function(permissionName) {
            ...
            $scope.saveRole = function() {
                if ($scope.isEdit===false) {
                    ...
                    $http.post('../websecurity/api/web-api/roles/create', $scope.role).
                       success(function() {
                       ModalService.motechAlert('security.create.role.saved', 'security.create');
                       $scope.roleList=Roles.query();
                       $scope.addRoleView=!$scope.addRoleView;
                       }).
                       error(function(){ModalService.motechAlert('security.create.role.error', 'server.error');});
                } else {
                    $http.post('../websecurity/api/web-api/roles/update', $scope.role).
                       success(function() {
                       ModalService.motechAlert('security.update.role.saved', 'security.update');
                       $scope.roleList=Roles.query();
                       $scope.addRoleView=!$scope.addRoleView;
                       $scope.$emit('module.list.refresh');
                       }).
                       error(function(){ModalService.motechAlert('security.update.role.error', 'server.error');});
                }
            };
            ...
            $scope.deleteRole = function() {
                ...
                success(function(){
                    ModalService.motechAlert('security.delete.role.saved', 'security.deleted');
                    $scope.addRoleView=!$scope.addRoleView;
                    $scope.roleList = Roles.query();
                }).error(function(response){
                    ModalService.handleResponse('server.error', 'security.delete.role.error', response);
                });
            };
            ...
            $scope.savePermission = function() {
                ...
                }, ModalService.angularHandler('server.error', 'security.create.permission.error'));
            };
                ...
                ModalService.confirm({
                    ...
                    callback: function(result) {
                        if (result) {
                            ...
                            }, ModalService.angularHandler('server.error', 'security.delete.permission.error'));
                        }
                    }
                });
            };
    ...
    controllers.controller('WebSecurityProfileCtrl', function ($scope, Users, $http, $routeParams, ModalService) {
        ...
        $scope.changeEmail = function () {
            $http.post('../websecurity/api/users/change/email', $scope.email).
                success(ModalService.alertHandler('security.update.email.saved', 'security.update')).
                error(ModalService.angularHandler('server.error', 'security.update.email.error'));
        };

        $scope.changePassword = function () {
            $http.post('../websecurity/api/users/change/password', [$scope.oldPassword, $scope.newPassword]).
                success(function () {
                    ModalService.motechAlert('security.update.userPass.saved', 'security.update');
                    delete $scope.user.oldPassword;
                    delete $scope.user.newPassword;
                    delete $scope.confirmPassword;
                }).error(function(response) {
                    ModalService.handleResponse('security.update.userPass.error', 'server.error', response);
                });
        };
    });

    controllers.controller('WebSecurityDynamicCtrl', function ($scope, Users, Permissions, Dynamic, ModalService) {
        ...
        $scope.save = function () {
            ...
            $scope.config.$save(
                ...
                function () {
                    ...
                    $("#dynamicURLSaveButtonBottom").button('reset');
                    $scope.savingDynamicURL = false;
                    ModalService.angularHandler('security.error', 'security.error.save');
                }
            );
        };
