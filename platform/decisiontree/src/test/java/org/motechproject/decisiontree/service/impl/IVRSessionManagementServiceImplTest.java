...
public class IVRSessionManagementServiceImplTest {
    ...
    public void shouldGetACallSessionRecordBasedOnSessionId() {
        ivrSessionManagementService.getSession("session1");

        verify(allFlowSessionRecords, times(1)).findOrCreate("session1");
    }

    @Test
    public void shouldUpdateAnExistingSessionRecord()  {
        FlowSessionRecord flowSessionRecord = new FlowSessionRecord("foo");
    ...
    public void shouldFigureOutWhetherASessionIsValidOrNot() {
        ...
        when(allFlowSessionRecords.findBySessionId("session2")).thenReturn(new FlowSessionRecord("session2"));
