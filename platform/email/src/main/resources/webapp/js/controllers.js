(function () {
    ...
    controllers.controller('EmailSendCtrl', function ($scope, SendEmailService, ModalServ) {
        $scope.mail = {};
        ...
        $scope.sendEmail = function () {
            if ($scope.mail.subject === undefined || $scope.mail.subject.length < 1) {
                ...
                SendEmailService.save(
                    ...
                    $scope.mail,
                    function () {
                        ModalServ.motechAlert('email.header.success', 'email.sent');
                    },
                    function (response) {
    ...
    controllers.controller('EmailSettingsCtrl', function ($scope, SettingsService, ModalServ) {
        ...
        $scope.add = function (property) {
            if ($scope.settings.additionalProperties[property.name] === undefined) {
                ...
                ModalServ.motechAlert('email.header.error', 'email.settings.alreadyExist');
            }
        };
        ...
        $scope.submit = function () {
            SettingsService.save(
                ...
                $scope.settings,
                function () {
                    ModalServ.motechAlert('email.header.success', 'email.settings.saved');
                    $scope.settings = SettingsService.get();
                },
                function (response) {
                    handleWithStackTrace('email.header.error', 'server.error', response);
