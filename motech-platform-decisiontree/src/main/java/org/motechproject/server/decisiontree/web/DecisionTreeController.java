...
public class DecisionTreeController extends MultiActionController {
    ...
    public ModelAndView node(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            return getModelViewForNextNode(request, encodedParentTransitionPath, params);
    ...
    private ModelAndView getModelViewForNextNode(HttpServletRequest request, String encodedParentTransitionPath, Map<String, Object> params) {
        String treeNameString = request.getParameter(TREE_NAME_PARAM);
        String transitionKey = request.getParameter(TRANSITION_KEY_PARAM);
        String[] treeNames = treeNameString.split(TREE_NAME_SEPARATOR);
        String rootNodeTransitionPath = TreeNodeLocator.PATH_DELIMITER;
        ...
        try {
            if (transitionKey == null) {
                node = decisionTreeService.getNode(currentTree, rootNodeTransitionPath, session);
                return constructModelViewForNode(request, node, rootNodeTransitionPath, treeNames, params, session);
            } else {
                ...
                if (emptyNode) {
                    ...
                    String modifiedTransitionPath = parentTransitionPath +
                            (rootNodeTransitionPath.equals(parentTransitionPath) ? "" : TreeNodeLocator.PATH_DELIMITER)
                            + transitionKey;
