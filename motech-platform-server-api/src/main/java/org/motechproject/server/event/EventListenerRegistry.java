...
/**
 * This class acts as a registry for all scheduled event listeners. One can
 * register themselves to listen for a specific set of event types.
 */
@Service
public class EventListenerRegistry implements EventListenerRegistryService {
     ...
     * @param subjects
     *            the event types that a listener is interested in
     */
    ...
    public void registerListener(EventListener listener, String subject) {
        ...
        metricsAgent.stopTimer("motech.listener-registry.addListener",
                startTime);
    }
     ...
     * 
     * @param subject
     *            The event type that you are seeking listeners for
     * @return A list of scheduled event listeners that are interested in that
     *         event
     */
    public Set<EventListener> getListeners(String subject) {
        final long startTime = metricsAgent.startTimer();
        Set<EventListener> ret = listenerTree.getListeners(subject);
        metricsAgent.stopTimer("motech.listener-registry.getListeners",
                startTime);
    ...
    public boolean hasListener(String subject) {
        ...
        metricsAgent.stopTimer("motech.listener-registry.hasListener",
                startTime);
