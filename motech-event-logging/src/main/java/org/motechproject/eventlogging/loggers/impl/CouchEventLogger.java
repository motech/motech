...
public class CouchEventLogger extends EventLogger {
    ...
    @Autowired
    public CouchEventLogger(AllCouchLogs allCouchLogs,
            DefaultCouchToLogConverter eventConverter) {
        this.allCouchLogs = allCouchLogs;
        this.eventConverter = eventConverter;
        this.loggableEvents = new ArrayList<LoggableEvent>();
    }
            ...
            if (loggableEvent.isLoggableEvent(eventToLog)) {
                if (eventConverter != null) {
                    ...
                    if (loggableEvent instanceof CouchLoggableEvent) {
                        couchLog = eventConverter
                                .configuredConvertEventToCouchLog(eventToLog,
                                        loggableEvent);
                    } else {
                        couchLog = eventConverter
                                .convertEventToCouchLog(eventToLog);
                    }
