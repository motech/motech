...
import org.motechproject.eventlogging.converter.EventToCouchLogConverter;
...
@Component
public class DefaultCouchToLogConverter implements EventToCouchLogConverter {

    @Override
    public CouchEventLog convertEventToCouchLog(MotechEvent eventToLog) {

        CouchEventLog couchEventLog = new CouchEventLog(
                eventToLog.getSubject(), eventToLog.getParameters(),
                DateTime.now());

        return couchEventLog;
    }

    public CouchEventLog configuredConvertEventToCouchLog(
            MotechEvent eventToLog, LoggableEvent loggableEvent) {
        ...
        for (KeyValue keyValue : keyValueList) {
            if (initialParameters.containsKey(keyValue.getStartKey())) {
                if (keyValue.getStartValue().equals(
                        initialParameters.get(keyValue.getStartKey()))) {
                    finalParameters.put(keyValue.getEndKey(),
        ...
        for (String includeParameter : inclusions) {
            if (initialParameters.containsKey(includeParameter)) {
                finalParameters.put(includeParameter,
                        initialParameters.get(includeParameter));
            }
        }

        CouchEventLog couchEventLog = new CouchEventLog(
                eventToLog.getSubject(), finalParameters, DateTime.now());
