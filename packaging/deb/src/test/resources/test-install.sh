...
MOTECH_OWNED="/var/lib/motech /var/log/motech /var/cache/motech /usr/share/motech/.motech"
NON_MOTECH_OWNED="/var/lib/motech /var/log/motech /var/cache/motech /usr/share/motech/.motech"
...
for dir in $MOTECH_OWNED" "$NON_MOTECH_OWNED; do
    $CHROOT file $dir >/dev/null # will return 0 if exists
    RET=$?
    if [ $RET -eq 0 ]; then
